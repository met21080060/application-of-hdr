...
def expand_integer_grid(arr, n_classes):
    ...
    """
    # Ensure arr contains integers within valid range
    arr_int = np.clip(np.round(arr).astype(np.int32), 0, n_classes-1)
    
    max_val = np.max(arr_int)
    
    if n_classes is None:
        n_classes = max_val + 1
    else:
        if max_val >= n_classes:
            n_classes = max_val + 1
            print(f"Warning: increasing n_classes to {n_classes} to accommodate array values")
    
    print(f"The length of hdr_image is: {len(arr)}")
    print(f"Max value in array: {max_val}")

    # Create the one-hot encoding
    one_hot = np.zeros(arr.shape + (n_classes,))
    
    # Use a safer approach to set the one-hot values
    # Create indices for all dimensions
    indices = np.indices(arr.shape)
    # For each element in the flattened array
    for idx in range(arr_int.size):
        # Convert flat index to multi-dimensional indices
        multi_idx = np.unravel_index(idx, arr.shape)
        # Get the value at this position
        val = arr_int[multi_idx]
        # Set the corresponding one-hot position to 1
        one_hot[multi_idx + (val,)] = 1
    
    # Verify the one-hot encoding is correct
    assert((one_hot.sum(-1) == 1).all())
    assert(np.allclose(np.argmax(one_hot, -1), arr_int))
...
def computeRadianceMap(images, log_exposure_times, response_curve, weighting_function):
    ...
    log_exp_np = np.array(log_exposure_times)
    
    # Reshape to 3D and broadcast
    log_exp_reshaped = log_exp_np.reshape(-1, 1, 1)
    lndT = log_exp_reshaped * np.ones(imgarr.shape)
